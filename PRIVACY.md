...
Every row below is a real telemetry channel that was active on vanilla GameHub 6.0.9 and is **no longer reachable** in the BannerHub v6 build that ships from `gamehub-609-build`. The "Mechanism" column says how, and "Merge commit" links to the actual code that did it. (These patches were carried forward from the 6.0.4 line and **re-fingerprinted against the 6.0.9 bytecode**. The mechanisms are unchanged **except the two `vgabc.com/events` kills**, which switched from the 6.0.4 fake-success-return to a **loopback URL redirect** — the 6.0.7+ result model made fabricating a fake success object crash-risky, so the senders now point at `http://127.0.0.1` instead. The commit links point at the original 6.0.4 implementation; the current anchors live in the [patch sources](https://github.com/The412Banner/bannerhub-revanced/tree/gamehub-609-build/patches/src/main/kotlin/app/revanced/patches/gamehub/misc/analytics).)
...
**6.0.9 device confirmation (2026-06-18, `v1.0.0-609`):** re-verified on the 6.0.9 stable with our own [DNSWatch](https://github.com/The412Banner/DNSWatch) app (per-app DNS / TLS-SNI / connection capture, root) monitoring the patched `banner.hub` build across a real session (launcher → catalog → game launch → in-game). **Zero** DNS queries or connections to `statistic-gamehub-api.vgabc.com` or any `*.vgabc.com/events` host — instead the capture shows a direct **`CONN OUT TCP 127.0.0.1:80`**, i.e. the analytics POST hitting the loopback redirect live. The only hosts the patched build reached were the BannerHub Worker (`bannerhub-api.the412banner.workers.dev`), GitHub / `img.shields.io` (Explore + badges), Steam store CDNs (`*.steamstatic.com`), content CDNs, and XiaoJi's functional app backend (`api-international-gamehub.xiaoji.com`, disclosed below) — **no patched telemetry channel leaked.**
...
*Last updated: 2026-06-18. Covers `gamehub-609-build` (GameHub 6.0.9, versionCode 121) — stable `v1.0.0-609`. The telemetry-kill patches are carried forward from the 6.0.4 line, re-fingerprinted against the 6.0.9 bytecode; the two `vgabc.com/events` kills use a loopback URL redirect (not the 6.0.4 fake-success return). **Device-confirmed on 6.0.9** via a live per-app DNS/SNI/connection capture (our [DNSWatch](https://github.com/The412Banner/DNSWatch) app): zero `vgabc.com/events` egress, analytics POST redirected to `127.0.0.1` (see the 6.0.9 device confirmation above). **Plan 11 (Disable Firebase auto-init)** remains in place — it closes a runtime Crashlytics-collection re-enable that overrode the manifest flag, removing the firebase-settings.crashlytics.com / firebaselogging-pa.googleapis.com / firebaseinstallations.googleapis.com residuals (originally device-confirmed on the 6.0.8 pre3 SNI trace; the patch re-applies + verifies on 6.0.9). The former Lite-only NumberAuth + cloud-gaming strips ship in every variant.*
