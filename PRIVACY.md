...
| --- | --- | --- | --- |
| **Firebase Analytics** | screen views, session starts, in-app purchases, app opens, custom events → `app-measurement.com` | Manifest `<meta-data>` kill switch (`firebase_analytics_collection_deactivated=true` + AD-ID/SSAID disables) — Analytics data collection never initializes (the Firebase SDK's ContentProvider still loads, which is why some Firebase *housekeeping* hosts remain — see residuals below) | [`178c5ec`](https://github.com/The412Banner/bannerhub-revanced/commit/178c5ec) |
...
Firebase's "Firelog" transport — the generic pipe Firebase SDKs (Crashlytics/Analytics/Perf) use to batch-ship events. **Device-confirmed present** in the 6.0.8 SNI trace (attributed to the app). With Analytics deactivated, Crashlytics collection off, and AD-ID stripped there's no event/crash/ad payload to carry, but the transport client itself still opens the connection. Out of scope for the *current* patches — Plan 10 disabled the GMS Measurement service path, not this separate logging path. The clean fix is **Plan 11 (disable Firebase auto-init)** described at the bottom of this file; until then, block it at the network level if you want it gone.

### `firebaseinstallations.googleapis.com`

Firebase Installations — registers a per-install Firebase ID (FID) on first launch. **Device-confirmed present** (intermittently) in the 6.0.8 trace. It's identity/registration housekeeping that every Firebase-linked app performs; it carries no analytics events, but the FID is an installation-scoped identifier. Same root cause as `firebaselogging-pa` (Firebase ContentProvider auto-init) and closed by the same Plan 11.

### `play.googleapis.com`

The generic **Google Play Services (GMS) API backend** — carries a grab-bag of GMS calls (Phenotype feature-config, FCM registration, Firebase-via-GMS, Play Integrity, etc.; the specific API is in the encrypted path). **Device-confirmed present** (one connection, attributed to the app, in the controlled 6.0.8 trace). Some of this is Firebase-driven (and so reduced by Plan 11); the rest originates from the GMS client library linked into the app and from system Play Services, **which is a separate system app we cannot patch**. Honest limitation: app-side patches can *reduce* this but may not drive it to zero without removing GMS entirely (which would break Google/Play-dependent paths). It carries no per-user analytics payload of ours. Block it at the network level only with care — it can break push (FCM) and Play Integrity.
...
## Planned hardening (the residuals above)

The Google residuals (`firebaselogging-pa`, `firebaseinstallations`, parts of `play.googleapis.com`) all share one root cause: the Firebase SDK's `FirebaseInitProvider` ContentProvider auto-initializes at process start, independent of any app code. Disabling collection (which we do) stops the *payloads* but not the *connections*.

- **Plan 11 — Disable Firebase auto-init.** Set `com.google.firebase.provider.FirebaseInitProvider` to `android:enabled="false"` in the manifest (and remove the `com.google.firebase.components.ComponentDiscoveryService` registration). With Analytics/Crashlytics already collection-disabled and no app code calling `FirebaseApp` directly, this should eliminate `firebaselogging-pa`, `firebaseinstallations`, and the Firebase-driven share of `play.googleapis.com` in one manifest change — pending an on-device check that the app still launches and that no functional feature relies on Firebase (FCM push is handled via the now-disabled Mob stack, not standalone Firebase). **Scoped, not yet shipped.**
- **GMS-side limits.** The non-Firebase share of `play.googleapis.com` / `userlocation.googleapis.com` originates from Google Play Services — a **separate system app we cannot patch**. App-side patches reduce but cannot guarantee zero. For a hard guarantee, block those two hosts at the network layer (per-app firewall / DNS sink); doing so can disable FCM push and Play Integrity, so weigh it against features you use.
