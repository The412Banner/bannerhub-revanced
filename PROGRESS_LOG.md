...
**Delivered for device test:** pre13 Normal APK → `/storage/emulated/0/Download/BannerHub-V6-1.1.0-608-pre13-Normal.apk` (md5 `db8c2b18959441554799100fe71f37fb`, 47,038,267 B, `banner.hub` Normal variant). Earlier pre11 / pre9 Normal also in Download.

**State:** branch pushed to `origin`, HEAD `7f8a432`, clean working tree. Not merged into `gamehub-608-build`; no stable cut. Memory: [[project_bannerhub_v6_steam_chat_overlay]]. NEXT: device-test pre13 bubbles + pinned back row, then iterate (unread badges / notification when overlay closed / friend search) or gate-merge.
