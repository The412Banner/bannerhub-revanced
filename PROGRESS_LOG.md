...
## 2026-05-16 — Lite branch brought current with preload-free vibration

`origin/gamehub-604-build` (`ed355d3`, incl. preload-free merge `72bb018`) merged into `feature/lite-variant-tier1` via `--no-ff` **merge commit `1b567ae`** (identity The412Banner, no Claude trailer), pushed `895f289..1b567ae`. Vibration files merged **cleanly** (lite branch never touched them): `VibrationLibPatch.kt` + `native/evshim/` removed, `VibrationPatch.kt` Hook 4 = `ensureWinebusDurationPatchOnce`, `BhVibrationController.java` carries the on-disk winebus patcher (braces 201/201). All 4 Lite strip patches intact (`StripDuplicateFontPatch`/`DisableNumberAuthPatch`/`StripCloudGamingPatch`/`StripImageCodecsPatch`); `release.yml` correctly retained BOTH the Lite variant matrix row (`extra: -e "Strip duplicate font" -e "Disable Aliyun NumberAuth" -e "Strip cloud gaming" -e "Strip AVIF/HEIC codecs"`) AND the libevshim-step removal. Only conflict = `PROGRESS_LOG.md` append-collision, resolved keeping both blocks. Lite rebuild triggered: Release run [25975305640](https://github.com/The412Banner/bannerhub-revanced/actions/runs/25975305640), `version=1.1.0-604-lite-vibpf-pre1`, artifact-only. → on green, pull the Lite (`gamehub.lite`) APK for device re-test (DiRT 3 boots no c000007b + rumble; Lite size-strips still applied).

**Lite rebuild GREEN** — Release run [25975305640](https://github.com/The412Banner/bannerhub-revanced/actions/runs/25975305640) success, all 9 variants incl. `Patch Lite`. `apk-Lite` = 75,598,665 B (≈75.6 MB — size-strips applied AND preload-free, no 41KB libevshim). Pulled to device: `/storage/emulated/0/Download/BannerHub-V6-Lite-vibpf-pre1.apk` (pkg `gamehub.lite`; user hand-renames to `banner.hub.lite` per usual). Lite branch `feature/lite-variant-tier1` @ `31ad6eb`. AWAITING device re-test: DiRT 3 + x86_64/32-bit titles boot (no c000007b) + rumble incl. sustained hold, WITH Lite size reductions intact.


## 2026-05-16 — STATE CHECKPOINT (preload-free vibration, both branches)

Closing state for the x86_64/box64 launch-death + vibration saga:
- **Root causes (both fixed):** (1) BannerHub-API `.wcp`-layout packaging of custom Box64/FEX comps → fixed/shipped (`bannerhub-api` `983fd47`); (2) residual `c000007b` = `VibrationPatch` Hook-4 libevshim LD_PRELOAD (BannerHub-introduced, NOT upstream) → preload-free rework.
- **`gamehub-604-build`** @ `ed355d3`: preload-free merge `72bb018` (CI+Release green, device-confirmed DiRT 3 boots + rumble).
...
## 2026-05-18 — Lite branch refreshed off gamehub-604-build (legacy renderer toggle)

`gamehub-604-build` (`0cd8423`, incl. renderer-toggle merge `01b2f4d`) merged into `feature/lite-variant-tier1` via `--no-ff` **merge commit `679763c`** (identity The412Banner, no Claude trailer), pushed `13ac017..679763c`. Brings legacy GLES2 renderer toggle (M2 per-game `libxserver` swap, M3 permanent forced-enable) + the 4 `renderer/` patches + `legacyrenderer/libwinemu_legacy.so` & `libxserver_legacy.so` into the Lite branch. 61 commits in, 16 Lite-only commits preserved (Tier 1–4 strips + `d3f63f7` 9-variant artifact-only build + docs). 2 merge conflicts, both resolved: `release.yml` (release-notes body only — kept the Lite-specific 9-Lite-variant table + retained the 604-side "Install notes" block; **build matrix / `stable!=true` artifact-only gate untouched, no conflict there**) and `PROGRESS_LOG.md` (append-collision — both blocks kept). Post-merge verified: 4 Lite strip patches present, all 9 Lite matrix rows keep the 4-strip `extra` flags, renderer patches + `_legacy.so` libs landed. Closes the legacy-renderer-toggle "refresh Lite" follow-up.

### Lite rebuild GREEN — run 26036859891 ✅
Artifact-only dispatch `release.yml --ref feature/lite-variant-tier1 -f version=1.3.0-604-lite-renderer-pre1` (stable defaulted false) on `560b568`. `conclusion=success`; all 9 `Patch *-Lite` jobs + bundle = success, `Create GitHub Release` = **skipped** (correct, artifact-only). Log-scanned all 9 jobs (2880 lines): **0 SEVERE**, 0 patch-failure signatures. Renderer toggle confirmed applied on Lite: `Legacy renderer conditional swap` / `Legacy renderer libxserver bundle` / `Renderer menu row` / `Renderer settings activity` / `Per-game menu id capture (shared)` all `succeeded`; 4 Lite strips (`Strip duplicate font` / `Disable Aliyun NumberAuth` / `Strip cloud gaming` / `Strip AVIF/HEIC codecs`) + GPU Spoof + PC-accurate vibration all `succeeded` — renderer + strips coexist cleanly. **APK delivered:** `/storage/emulated/0/Download/BannerHub-V6-1.3.0-604-lite-renderer-pre1-Patched-Normal-Lite.apk` (pkg `banner.hub`, 79,840,335 B ≈76.1 MB, md5 `aa2e7538f34a4cc98f40154a799fb6e7`). **M3 DEVICE-TEST PASSED (2026-05-18, user-confirmed):** on the user's actual `banner.hub` **Lite** install, GoW boots with renderer **Legacy** AND with **Vulkan** — bidirectional verification on the real Lite install (not just the full-pkg proxy). This closes the sole remaining gap. **Legacy GLES2 renderer toggle is now FEATURE-COMPLETE: merged to `gamehub-604-build` (`01b2f4d`), refreshed into Lite (`679763c`), Lite rebuild green (run 26036859891), and device-verified bidirectionally on both full-pkg (`com.antutu.benchmark.full`) and the user's `banner.hub` Lite.** No open items.

---

## Offline picker DIAGNOSTIC build — delivered, awaiting offline device test (2026-05-18)

Prefs-read fix (run 26039944075) device-tested → **FAILED**: pickers still built-in only, ZERO `PickerCacheFallback`/`GH600-DEBUG` trace. Verified (root + fresh 6.0.4 + delivered-APK decompile): patch injects correctly; `p1` IS RepoCategory at the call site (continuation slots L$0/L$1 traced); host+ours both MODE_PRIVATE same prefs; no separate `android:process`. `RepoCategory` has only `COMPONENT`/`CONTAINER`/`IMAGE_FS` (no GPU_DRIVER/DXVK/etc — those are COMPONENT sub-typed by `entry.fileType`); on-disk prefs = 535 COMPONENT + 10 CONTAINER + 1 IMAGE_FS. Leading unconfirmed cause: offline the fetch THROWS and the coroutine rethrows at `Lkl5;->V` (throwOnFailure) on resume **before** `:goto_2` → the patch's whole "offline → EmptyList return" premise is wrong.

**Diagnostic (user chose diagnostic-first):** target corrected from `gamehub.lite` (not installed / no components → false negative) to **`banner.hub` Normal-Lite** (real install w/ 535 components, no AnTuTu payload → flood-free). Branch `diag/offline-picker-probe` off `feature/lite-variant-tier1` (throwaway, never merge); carries the prefs-read fix + new `OfflineDiag` internal-files sink (NOT DebugTrace — the suspected blind spot) + throwaway `OfflineDiagProbePatch` injecting `mciEnter()`@mci.a[0] and `mciPostResume()` after the resume-path `Lkl5;->V`; `fromXxo` marks true-entry. **CI run 26041576477 GREEN, 0 SEVERE; probes VERIFIED by decompiling the DELIVERED APK** (OfflineDiag bundled; mciEnter@idx0; mciPostResume right after 1st Lkl5;->V@111→113; fromXxo@218). **APK:** `/storage/emulated/0/Download/BannerHub-V6-1.3.0-604-offlinediag-pre1-Patched-Normal-Lite.apk` (md5 `203e35ba0e9df45ef8d224868ccac2e8`, pkg `banner.hub`, same key → in-place, components preserved). **AWAITING USER OFFLINE TEST** (install, airplane, open each picker), then pull `/data/data/banner.hub/files/bh_offline_diag.log` via root. Interpretation: none=separate path; ENTER-only=threw before :goto_2 (premise wrong); ENTER+POST_RESUME no fromXxo=branched away; all three=DebugTrace was blind spot. NOT merged; KNOWN BROKEN stands.

### DIAGNOSTIC RESULT — mci.a is the WRONG hook for 6.0.4 (2026-05-18, DECISIVE)

User offline-tested `1.3.0-604-offlinediag-pre1` (banner.hub Normal-Lite), opened every picker. Root pull: `/data/data/banner.hub/files/bh_offline_diag.log` **does not exist**; zero `BH-OFFLINE-DIAG` logcat; no exceptions. Build confirmed installed & run — `files/profileInstalled` + `profileinstaller_profileWrittenFor_lastUpdateTime.dat` written 11:11 (package update + first-launch), `xj_winemu/` + `sp_winemu_unified_resources.xml`@09:46 preserved = successful in-place update (signature matched, components intact). `mciEnter()` is `mci.a` index 0 → file absent ⇒ **`mci.a` never invoked once** while opening GPU driver/DXVK/VKD3D/FEXCore/Box64/container offline.

**Conclusion ("none" case): 6.0.4 component pickers do NOT use `mci.a`.** `OfflineComponentCachePatch`'s entire `eci.a`/`mci.a` premise is a 6.0.2-era assumption that does not hold in 6.0.4 — the patch has been hooking a dead method, **inert since inception, not a 6.0.2→6.0.4 regression**. Read-side at `mci.a` is dead; prior H1/H2/H3 analysis was about the wrong method.

**STRATEGY RESET — effectively a from-scratch patch.** Next: fresh investigation of what actually populates the 6.0.4 picker dropdowns (picker Compose screen → ViewModel → data source; likely in-memory `myo`/`w0j` registry or downloaded-file enumeration; GPU-driver "system driver only" may be a separate AdrenoTools path). Branches `fix/offline-component-picker` (prefs-read, failed) + `diag/offline-picker-probe` (throwaway) stay unmerged; KNOWN BROKEN banner stays. Reusable: `OfflineDiag` sink + the verify-injection-by-decompiling-delivered-APK + banner.hub-Normal-Lite-flood-free + root-pull workflow.

### Diagnostic pre2 (myo.w + j7o probes) — delivered, awaiting offline test (2026-05-18)

mci.a proven dead. Re-targeted probes to the real architecture: `myo.w(RepoCategory)→ArrayList` (picker list source, filters in-memory `myo.c`, no disk) + `j7o.<init>` (repo ctor holding the disk hydrator `dj9.a().getAll()`→Gson→`ici.e`→`map.put`). CI run 26042994877 GREEN, 0 SEVERE, patches succeeded. **Probes VERIFIED in delivered APK** (located winemu classes by signature, not path — R8 collisions): `myoW()`@myo.w[0]; `myoWReturn(v0)` before `return-object v0`; `j7oCtor()` injected right after `j7o.<init>`'s `Ljava/lang/Object;-><init>()V` super-call (verifier-safe). APK `/storage/emulated/0/Download/BannerHub-V6-1.3.0-604-offlinediag-pre2-Patched-Normal-Lite.apk` (md5 `dd767356dca9cafd086a2c7e26b0d16d`, pkg `banner.hub`, in-place). AWAITING user offline test → pull `/data/data/banner.hub/files/bh_offline_diag.log` via root. Decode: `myo.w ENTER`+size>0 → map populated, failure downstream of picker; ENTER+size=0 +`j7o.<init>` ran → hydrator didn't fill map (or too late); ENTER+size=0 + no `j7o.<init>` → repo not constructed pre-picker → fix = hook myo.w to read disk (existing prefs+Gson logic, finally correct method); no `myo.w ENTER` → yet another feed.

### Diagnostic pre2 result + METHODOLOGY WALL (2026-05-18)

pre2 (myo.w + j7o.<init> probes, verified injected in delivered APK) offline-tested → **again ZERO OfflineDiag output**, including `j7oCtor()` (the central WinEmu repo ctor that must run in normal use). Third total silence. **The instrument is now untrustworthy:** renderer diag writes fine on this device (file-sink works), so either OfflineDiag.ctx() (ActivityThread.currentApplication()) fails at probe sites, or the running code isn't our instrumented build. Root-bridge limits confirmed: `/data/app`, `pm`, `dumpsys` all blocked (allowlist = `/data/data/<pkg>/`, sqlite, `/proc/<pid>`, logcat only); `banner.hub` not currently running → cannot confirm build identity or live-introspect. **Consequence:** "mci.a is dead" and "myo.w not the path" are NOT reliable conclusions. Static architecture finding (myo.w = picker list source, j7o = disk hydrator) stands (code-structure, sink-independent). Read-side instrument-and-read-sink loop has failed 3× and is blocked by build-identity unverifiability. Decision checkpoint raised with user (root-only data-layer investigation vs pause vs reconsider approach). NOT merged; KNOWN BROKEN stands.

### PAUSED — logcat-bridge v1.4.0 built, awaiting flash+reboot (2026-05-18)

Read-side instrument loop hit a wall: 3 diag builds → zero OfflineDiag output; could not confirm whether the user's running build is our instrumented APK because the bridge blocked `/data/app`/`pm`/`dumpsys`. Fix: upgraded logcat-bridge → **v1.4.0** (arbitrary-root `exec|sh` verb + cat/ls/sql/write allowlist broadened to ANY absolute path; token gate kept). Flashable zip `/storage/emulated/0/Download/logcat-bridge-magisk-v1.4.0.zip` (md5 `8947f25eb1694779f992a02461a38a41`); client `getlog` already on PATH. User will flash (Magisk → Modules → Install from storage) + reboot. **On resume:** `project_bannerhub_revanced_offline_component_cache.md` → `▶▶ RESUME AFTER REBOOT` section is the exact step list (confirm bridge → `getlog --cat` the installed base.apk + apktool → settle is-our-code-running → branch fix accordingly → live-root map of picker path). Static architecture finding (picker = `myo.w` over in-memory `myo.c`; hydrator = `j7o.<init>` from `sp_winemu_unified_resources`) STANDS. "mci.a dead" / "myo.w not called" device-negatives UNTRUSTED until build-identity settled. Branches `diag/offline-picker-probe`@`8a5903d` + `fix/offline-component-picker` — NOTHING merged, KNOWN BROKEN banner stands.
