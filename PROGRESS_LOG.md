...
Pure ext Java (controller +234/−13, activity +24); no `.kt` patch / CVR / asset / anchor change; brace-balanced. Build = artifact-only `release.yml` pre11. Retest: (1) DX9/10/11-on-DXVK still works (regression check), (2) toggle on → a DX12/VKD3D title sees the spoofed GPU, (3) wined3d-renderer title sees it, (4) frame-gen indeed off when deep on.
