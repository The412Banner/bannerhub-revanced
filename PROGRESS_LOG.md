...
**State:** branch pushed to `origin`, HEAD (post-log) on `feature/steam-chat-overlay`, clean working tree. Not merged into `gamehub-608-build`; no stable cut. Memory: [[project_bannerhub_v6_steam_chat_overlay]]. NEXT: device-test pre18 (Overlay tile opens w/o root → 2 toggles; perf disabled+grant-link when no root; chat pill on right, draggable, opacity slider), then optionally friend search / typing indicator (`friends.send_typing`) / stickers, or **gate-merge to `gamehub-608-build` + cut stable**.
