...
**Delivered for device test:** pre17 Normal APK → `/storage/emulated/0/Download/BannerHub-V6-1.1.0-608-pre17-Normal.apk` (md5 `d0681e5421981e59a80c8f653948d83f`, `banner.hub` Normal variant). pre16…pre9 Normal also in Download.
...
**State:** branch pushed to `origin`, HEAD (post-log) on `feature/steam-chat-overlay`, clean working tree. Not merged into `gamehub-608-build`; no stable cut. Memory: [[project_bannerhub_v6_steam_chat_overlay]]. NEXT: device-test pre17 mark-read (badge clears + stays cleared), then optionally friend search / typing indicator (`friends.send_typing`) / stickers (`friends.chat_stickers`/`send_sticker`), or **gate-merge to `gamehub-608-build` + cut stable**.
