...
**State:** branch pushed to `origin`, HEAD (post-log) on `feature/steam-chat-overlay`, clean working tree. Not merged into `gamehub-608-build`; no stable cut. Memory: [[project_bannerhub_v6_steam_chat_overlay]]. NEXT: device-test pre16 (avatars/badges/send-state; confirm outgoing avatar is *mine* not the friend's), then optionally friend search / typing indicator (`friends.send_typing`) / stickers / mark-conversation-read on open, or gate-merge.
