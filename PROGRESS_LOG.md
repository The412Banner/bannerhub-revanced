...
- **pre15** `5722163` — **grouped name + timestamp headers (native-style).** New `senderHeader(who, ts, fromMe)` line shown once per sender run (groups consecutive messages like GameHub's native chat): friend `displayName` (blue) left / "You" (green) right, with the time. Timestamp = the `timestamp` field, an **unsigned-32 Unix-epoch-seconds UInt** (`fwk` value class masks `& 0xFFFFFFFF`); `formatTime()` × 1000 → `android.text.format.DateFormat.getTimeFormat(act)` (device 12/24h). Build run `27108156680` ✅, all 9 variants, no SEVERE.

**Delivered for device test:** pre15 Normal APK → `/storage/emulated/0/Download/BannerHub-V6-1.1.0-608-pre15-Normal.apk` (md5 `632c2b8a35f79d5f3a1d3a2e8bf4dcef`, `banner.hub` Normal variant). pre14/pre13/pre11/pre9 Normal also in Download.
...
**State:** branch pushed to `origin`, HEAD `5722163`, clean working tree. Not merged into `gamehub-608-build`; no stable cut. Memory: [[project_bannerhub_v6_steam_chat_overlay]]. NEXT: device-test pre15 name/time headers, then iterate (avatars — `avatarUrl` is in the schema / unread badges via `unread` / `sendState` "Sending" indicator / friend search) or gate-merge.
