...
**GOG card logo (this commit):** the Explore GOG card showed a plain purple placeholder chip. Made the card icon data-driven — `BhExploreManifest.Card` gains an optional `icon` field (android drawable NAME, resolved at runtime via `getResources().getIdentifier(icon,"drawable",pkg)`), `BannerExploreActivity.buildCard` renders an `ImageView` (FIT_CENTER, 56dp) when resolvable else the accent chip (never throws). New `ExploreDrawablesPatch` (resourcePatch, mirrors BannerToolsDrawablesPatch) copies `patches/src/main/resources/explore/bh_explore_gog.png` (user-provided GOG Galaxy macOS-BigSur icon, 512×512 RGBA, 34 KB) → `res/drawable/bh_explore_gog.png`. BUNDLED_JSON GOG card now `"icon":"bh_explore_gog"`. Build PENDING.
