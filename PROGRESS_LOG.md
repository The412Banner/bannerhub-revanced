...
**Cut:** `release.yml` `stable=true` on `gamehub-604-build` (`version=1.6.0-604`) creates the GH Release with 9 full variants; then on `feature/lite-variant-tier1` (`version=1.6.0-604`) appends 9 Lite variants to the same `v1.6.0-604` tag (softprops, same-tag append). End state = 21 assets. Run IDs + tag recorded after CI. Pre-release policy ([[feedback_bannerhub_prerelease]]) resumes for all subsequent builds.
