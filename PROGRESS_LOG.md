...
**Build:** gog-pre25, artifact-only (pre-release). Device test PENDING: tap Explore in BOTH modes → our screen; GOG card → GogMainActivity; cold start lands on normal home (no hijack misfire). Watch `getlog gamehub.lite | grep BhExploreTab`. Branch still isolated from 604/Lite per directive.
