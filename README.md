...
Normal and Original share the bare "BannerHub v6" launcher label and the two AnTuTu variants share "BannerHub v6 AnTuTu" — they install side-by-side via different package names, so the shared labels are intentional. The **Lite** variant (`gamehub.lite`) is the same patch bundle minus a verified-dead duplicate 20 MB font, the Aliyun carrier-login native lib, the Haima cloud-gaming stack, and the avif-coil AVIF/HEIC image-codec stack — ~42 MB smaller on disk. Removed features: cloud gaming (non-functional under the BannerHub catalog redirect anyway) and the bundled AVIF/HEIC decoder (modern Android renders HEIF/AVIF via the platform decoder; JPEG/PNG/WebP unaffected). Everything else is byte-identical to Normal. Part of an incremental size-reduction track.
