...
How to configure **Beacon**, **ES-DE**, **RetroHRAI**, **NeoStation**, and **Daijishou** to launch games into any of BannerHub v6's nine variants on the 6.0.4 base. All five share the same Android intent contract — only the per-launcher **placeholder syntax** for the ROM-file value differs (Beacon-family vs. Daijishou-family — see [Placeholder syntax by front-end](#placeholder-syntax-by-front-end) below).
...
> ✅ **NeoStation — device-confirmed working** (the412banner, v1.5.1-604, 2026-05-22). Ships built-in BannerHub v6 launch entries — no custom `am start` editing needed; just drop a `<game>.localgameid` file in your `windows/` ROM folder and pick "Standalone BannerHub v6 (Normal)" (or your matching variant) from the emulator picker.
...
> 📺 **Video walkthroughs** —
> • **Beacon** (covers creating PC-import game `.txt` / `.iso` files with GameID numbers): <https://youtu.be/hyjjs-ffpw4?si=Lp6CCGhwFKGR0tAA>
...
| **NeoStation** | `{tags.localgameid}` | Same family as RetroHRAI/Daijishou — extension-to-tag mapping driven by the system JSON. The `windows` system needs `"localgameid"` registered in its `extensions` array (added via [`miguelsotobaez/neostation-systems` PR #9](https://github.com/miguelsotobaez/neostation-systems/pull/9) on top of the BannerHub v6 launch rows from [PR #7](https://github.com/miguelsotobaez/neostation-systems/pull/7)); trigger NeoStation's systems-sync once both have landed to pick up the new rows. |
...
> ⚠️ Do **not** split commands across multiple lines with `\` continuations — Beacon / RetroHRAI / NeoStation fields treat the whole thing as one command and the backslashes/newlines will break the launch. Paste as a single line.

The shell-reference blocks below come in two flavors:

- **Beacon / ES-DE** — `{file_content}` placeholder
- **RetroHRAI / NeoStation / Daijishou** — `{tags.localgameid}` placeholder
...
## Shell reference (per-variant `am` examples) — RetroHRAI / NeoStation / Daijishou
...
> 🛰️ **NeoStation users don't need these blocks** — the 9 BannerHub v6 launch rows are already in NeoStation's bundled `systems/windows.json` (upstream PR #7). Once [PR #9](https://github.com/miguelsotobaez/neostation-systems/pull/9) adds `localgameid` to the windows extensions array, drop `<game>.localgameid` files in your `windows/` folder, trigger NeoStation's systems-sync, and pick "Standalone BannerHub v6 (Normal)" (or your matching variant) from the emulator picker. Each `.localgameid` file's content is the integer game ID (read via Banner Tools → Show Game ID). The blocks below are useful only if you want to override the bundled launch command.

> 📁 **ROM folder + file contents are identical to Beacon's** — each `.txt` / `.iso` / `.localgameid` file's content is the integer `localGameId` for that game (read via Banner Tools → Show Game ID). Beacon, RetroHRAI, and NeoStation can share the same folder.
