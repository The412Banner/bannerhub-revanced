...
**Status:** ⚑ **GREENLIT. PHASE 0 COMPLETE — CURRENT = §20.** Re-scoped per user: prove **GOG login + library + download** first (PHASE 1, LOW-risk, executable); defer GameHub-library/launch bridge + production entry-point (PHASE 2). Source = lift `bannerhub/extension/Gog*.java` (3.7.x, **zero-third-party-dep pure-platform Java — P-D GREEN, dep-clash risk eliminated**) into `extensions/gamehub/`, NOT a GameNative port. Phase-0: P-B = launcher-onResume hook ports to `MainActivity` but 5.3.5 `B3()` reflection dead on 6.0.4 KMP → bridge = programmatic `xm7.u` import (§19, **deferred**); P-A (Profile-row anchor)→WS4 deferred; P-C→WS5 deferred. **Next = PHASE 1/WS1: port `Gog*.java`, stand up OAuth+library activities behind a temp entry; first code.** §1–§19 = trace record. Branch `feature/gog-explore-tab`.
