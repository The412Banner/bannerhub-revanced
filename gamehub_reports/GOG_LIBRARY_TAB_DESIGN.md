...
**Status:** ⚑ **WS1 FIRST-CUT DONE — CURRENT = §22. UNVERIFIED (CI-only).** Phase 1 (GOG login+library+download, standalone) code committed on `feature/gog-explore-tab` (`3f7ce63`): 19 GOG-only files ported `→app.revanced.extension.gamehub.gog`, decoupled from the store-entangled Epic/Amazon shared infra, `GogLaunchHelper` Phase-1-stubbed, `GogManifestPatch.kt` registers 6 activities (`GogMainActivity` exported = temp `adb` dev entry). Inspection-clean (self-contained closure, balanced braces, no unresolved refs) but **not compiled — first CI build = the M1 gate.** Deferred Phase 2: GameHub-library/launch bridge (§19, programmatic `xm7.u`), production Profile-row entry (WS4/P-A), P-C. **Next = trigger CI build of the branch → on green, device-test M1 (GOG login).** §1–§21 = trace+scope record. Base GameHub 6.0.4.
